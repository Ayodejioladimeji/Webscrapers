        ...
        try {
            ...
            if (error.message.includes('Navigating frame was detached') || error.message.includes('Timeout')) {
                if (i < retries - 1) {
                    ...
                    await setTimeout(5000); // Use Node.js setTimeout for delay
                }
            } else {
...
// MODIFIED: Added startPage and endPage parameters
export async function scrapeListingsFromURL(
    initialUrl: string,
    startPage: number = 1,
    endPage: number = Infinity // Use Infinity to scrape all pages by default
): Promise<ScrapedListing[]> {
    const allBasicListings: ScrapedListing[] = [];
    ...
    try {
        ...
        let pageCounter = startPage; // Start from the specified page

        // Construct the initial URL for the starting page if it's not page 1
        if (startPage > 1) {
        ...
        // MODIFIED: Added condition to stop if endPage is reached
        ...
        for (const listing of allBasicListings) {
            if (listing.listingUrl) {
                const detailData = await scrapeListingDetails(detailPage, listing.listingUrl);
                const fullListing = { ...listing, ...detailData };
        ...
        console.log(`Scraped ${fullListings.length} total full listings from ${initialUrl}`);
        return fullListings;
